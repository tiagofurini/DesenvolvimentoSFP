<?php
			...
			&& in_array($extension, $allowedExts))
			  {
			      ...
			      "padroeiro/".$id."padroeiro.".$extension);
			  }
			else
			  {
	...
	if(isset($_GET['param'])){
		...
		$SQL = "SELECT * FROM padroeiro WHERE pad_id=".$param;
		$execute_query = mysql_query($SQL, $conn); 
		if($query = @mysql_fetch_array($execute_query)){ 
			$titulo = $query['pad_titulo'];
			$descricao = $query['pad_descricao'];
			$foto = $query['pad_foto'];
		} 
	}
	...
	if(isset($_POST['Botao'])){
		...
		if($botao=="Cadastrar" AND $mensagem==""){
			$SQL = "INSERT INTO padroeiro(pad_titulo, pad_descricao) VALUES ('$titulo','$descricao')";
			$execute_query = mysql_query($SQL, $conn);
			if($execute_query){
				...
				if(!$mensagem){
					...
					$extension = end($temp);
					$img =  $ultimo_id."padroeiro.".$extension;
					$SQL = "UPDATE padroeiro SET";
					$SQL = $SQL. " pad_foto='".$img."'";
					$SQL = $SQL. " WHERE pad_id=".$ultimo_id;
		...
		//Atualizando Registro ****************************************************************************************************
			...
			if($foto["error"] == 0){
				$temp = explode(".", $foto["name"]);
				$extension = end($temp);
				$img =  $codigo."padroeiro.".$extension;
				$SQL = "UPDATE padroeiro SET pad_titulo='$titulo'";					  
				$SQL = $SQL. ", pad_descricao='$descricao'";
				$SQL = $SQL. ", pad_foto='$img'";
				$SQL = $SQL. " WHERE pad_id=".$codigo;
		...
		//Excluindo Registro ****************************************************************************************************
			...
			$codigo = (int)$_POST['txtId'];
			echo $codigo;
			$SQL = "DELETE FROM padroeiro WHERE pad_id=".$codigo;
			$execute = mysql_query($SQL, $conn);						
			if($execute){
				$botao = "";
				$titulo = "";
...
<html lang="en">
	...
	<body>
		<div id="container">
			...
			<div id="content">
				<div id="pagina"> 
					GERENCIAR PADROEIRO<br />
				</div>
				<br clear="all" />
				<form method="post" action="padroeiro_cad.php" enctype="multipart/form-data">
					<?php if(isset($param)){ ?>
						...
						<tr>
							...
							<td class="coluna2">
								...
								<?php if(isset($param) AND $foto!=""){ ?>
									<img src="padroeiro/<?=$foto?>" style="margin-top: 5px; height: 100px;"/>
								<?php }?>
							</td>
						</tr>
					</table>
