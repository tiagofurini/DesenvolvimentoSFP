<?php
	...
	$busca="";
	$SQL = "SELECT * FROM noticias ORDER BY not_data DESC";
		...
		<div id="container">
			...
			<div id="content">
				<div id="pagina"> 
					...
					<a href="noticias_cad.php"> <img src="css/img/cadastrar.png" style="margin-top: 10px;"/></a>
				</div>
				<br clear="all" />
				<table id="consulta">
					<thead>
					...
					if($busca=="ok"){
						while($query = @mysql_fetch_array($execute_query)){
							$separador = explode("-", $query['not_data']);
							$data =$separador[2]."/".$separador[1]."/".$separador[0];
							if($cont%2==0){ 
								echo "<tr class='odd'>";
							}else{ 
								echo "<tr>";
    					 	}?>
								<td style="text-align: left;"><?=$query['not_titulo']?></td>
								<td><?=$data?></td>
								<td><a href="noticias_cad.php?param=<?=$query['not_id']?>"><img src="css/img/editar.png" /></a> </td>
							</tr>
					<?php $cont++; }
					}else{?>
						<tr>
